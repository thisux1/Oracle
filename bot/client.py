            ...
            if msg_clean.startswith("sb "):
                ...
                if cmd in ["pause", "stop"]:
                    ...
                    tutorial_msg = (
                        ...
                        "• `sb tc start [minutos]m`: Ativa o modo Time Cookie por determinado tempo(ex: `sb tc start 60m`).\n"
                        "• `sb tc start`: Ativa o modo Time Cookie até seu comando de pause/stop.\n"
                        ...
                        "• `channel_id` / `guild_id`: Define de onde ler e enviar comandos. *Fundamental para o bot operar.*\n"
                        "• `admin_ids`: IDs autorizados a usar os comandos `sb`. *Sem isso, você não controla o bot pelo Discord.*\n"
                        "• `work_command` / `seed` / `lootbox_type`: Suas ações padrão (ex: `fish`, `potato`, `ed lb`).\n"
                        "• `is_married` / `is_ascended` / `partner_name`: Habilita comandos `hunt t` ou `hunt h` e rastreia loot do cônjuge.\n"
                        "• `telegram_bot_token` / `chat_id`: Alertas de Captcha e Prisão no celular. *Segurança anti-ban primária.*\n"
                        "• `random_interval`: Adiciona micro-atrasos humanos. *Evita detecção por padronização.*\n"
                        "• `tc_stop_on`: Condição para parar o Time Cookie (ex: `dungeon`).\n"
                        "• `current_area` / `zombie_horde_event_response`: Sobrevivência ao evento Zumbi (ex: mandar `fight` se na área certa).\n"
                        "• `daysToCloseVoid`: Resposta do treinamento no Void.\n"
                        "• `bankroll`, `max_losses`, `initial_step`: Configurações do cassino (Fibonacci).\n"
                        "• `typo_chance`: Chance (ex: 0.05 para 5%) de errar a digitação e corrigir em seguida. *Evita detecção.*"
                    )
