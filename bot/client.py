            ...
            if msg_clean.startswith("sb "):
                ...
                if cmd in ["pause", "stop"]:
                    ...
                    tutorial_msg = (
                        "📚 **Tutorial Oracle v2**\n\n"
                        "**Comandos Manuais:**\n"
                        "• `sb start` / `sb resume`: Inicia/retoma o bot.\n"
                        "• `sb pause` / `sb stop`: Pausa o bot.\n"
                        "• `sb reset`: Limpa as filas de comandos e variáveis (pets, gambling, etc).\n"
                        "• `sb tc start [Xc] [minutos]m`: Ativa TC mode. Ex: `sb tc start 4c 60m` (4 cookies, 60 min).\n"
                        "• `sb tc start`: Ativa TC mode até pause/stop (enfileira comandos iniciais).\n"
                        "• `sb tc pause` / `sb tc stop`: Desativa o modo Time Cookie.\n"
                        "• `sb g start`: Inicia o modo gambling (Coinflip Fibonacci).\n"
                        "• `sb g pause`: Pausa o modo gambling.\n"
                        "• `sb stats`: Mostra o progresso atual, loots e status.\n"
                        "• `sb say [texto]`: Envia uma mensagem no canal.\n\n"
                        "**Personalizações (options.ini):**\n"
                        "• `do_hunt`, `do_adv`, `do_farm`, `do_work`, etc: Habilita/desabilita comandos individuais.\n"
                        "• `do_ultr`: Substitui training pela sequência ULTR (ultr → double → attack → rpg use tc).\n"
                        "• `is_eternal`: Ativa auto-enter em dungeon e bite loop no dragão eternal.\n"
                        "• `card_hand_action`: `auto` (joga automaticamente) ou `notify` (só notifica Telegram).\n"
                        "• `tc_quantity`: Quantidade padrão de time cookies por uso (sobrescrito via `sb tc start Xc`).\n"
                        "• `life_boost_before_adv` / `adventure_area`: Compra life boost e troca de área antes de adventure.\n"
                        "• `user_id`: Seu ID do Discord. *Fundamental para o bot criar suas menções.*\n"
                        "• `channel_id` / `guild_id`: Define de onde ler e enviar comandos.\n"
                        "• `admin_ids`: IDs autorizados a usar os comandos `sb`.\n"
                        "• `work_command` / `seed` / `lootbox_type`: Ações padrão (ex: `fish`, `potato`, `ed lb`).\n"
                        "• `is_married` / `is_ascended` / `partner_name`: Comandos `hunt t`/`hunt h` e loot do cônjuge.\n"
                        "• `telegram_bot_token` / `chat_id`: Alertas de Captcha e Prisão.\n"
                        "• `random_interval`: Micro-atrasos humanos anti-detecção.\n"
                        "• `tc_stop_on`: Condição para parar TC (ex: `dungeon`).\n"
                        "• `current_area` / `zombie_horde_event_response`: Sobrevivência a eventos.\n"
                        "• `daysToCloseVoid`: Resposta do treinamento no Void.\n"
                        "• `bankroll`, `max_losses`, `initial_step`: Configurações do cassino.\n"
                        "• `typo_chance`: Chance de errar digitação (ex: 0.05)."
                    )
