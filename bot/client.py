            ...
            if msg_clean.startswith("sb "):
                ...
                if cmd in ["pause", "stop"]:
                    ...
                    tutorial_msg = (
                        "📚 **Tutorial Oracle v2**\n\n"
                        "**Comandos:**\n"
                        "• `sb start/pause`: Inicia/Pausa o bot\n"
                        "• `sb reset`: Limpa filas e estado\n"
                        "• `sb tc start [Xc][m]`: Ativa TC. Ex: `sb tc start 4c 60m`\n"
                        "• `sb tc stop`: Desativa TC\n"
                        "• `sb g start/pause`: Inicia/Pausa gambling\n"
                        "• `sb stats [tempo]`: Status da sessão\n"
                        "• `sb say [texto]`: Envia mensagem\n\n"
                        "**options.ini:**\n"
                        "• `do_hunt/adv/farm/work/etc`: Liga/Desliga comandos\n"
                        "• `do_ultr`: Sequência ULTR (ultr→double→attack→rpg use tc)\n"
                        "• `is_eternal`: Dungeon auto + dragão eternal\n"
                        "• `card_hand_action`: auto ou notify\n"
                        "• `tc_quantity`: Cookies por uso (ou `sb tc start Xc`)\n"
                        "• `life_boost_before_adv`: Life boost antes de adv\n"
                        "• `adventure_area` / `current_area`: Troca de área no adv\n"
                        "• `user_id/channel_id/guild_id/admin_ids`: Essenciais\n"
                        "• `work_command/seed/lootbox_type`: Comandos padrão\n"
                        "• `is_married/is_ascended/partner_name`: Casado/ascendido\n"
                        "• `telegram_bot_token/chat_id`: Alertas Telegram\n"
                        "• `random_interval/typo_chance`: Anti-detecção\n"
                        "• `tc_stop_on`: Para TC em evento (ex: dungeon)\n"
                        "• `current_area/zombie_horde_event_response`: Eventos\n"
                        "• `bankroll/max_losses/initial_step`: Gambling\n"
                        "• `daysToCloseVoid`: Resposta Void"
                    )
